...
Provides:
  ...
  - load_from_bq()   : read a BQ table into a DataFrame
  - ask_bq_agent()   : call Google's Conversational Analytics API against
                       a pre-configured BigQuery data agent
...
def ask_bq_agent(
    ...
    (geminidataanalytics.googleapis.com), targeting the pre-configured
    BigQuery data agent so its system instructions + glossary are honored.
    ...
    payload = {
        ...
        "data_agent_context": {
            "data_agent": f"projects/{project_id}/locations/global/dataAgents/{agent_id}",
        },
    }
